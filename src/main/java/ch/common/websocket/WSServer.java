...
public class WSServer {
     ...
     *
     * @param message 消息对象{@link }的JSON对象
    ...
    public void onMessage(String message) throws IOException {
        ...
        WSServer _client = map.get(msg.getToId());

        // 定点发送
        if (StringUtils.isNotEmpty(msg.getToId())) {
            if (null != _client) {
                // 是否连接判断
                if (_client.session.isOpen())
                    // 消息发送
                    _client.session.getBasicRemote().sendText(msg.getData());
            }
        }
    ...
    public void pushBySys(Msg msg) {
        ...
        WSServer ws=map.get(msg.getToId());
        try {
            if(ws!=null){
