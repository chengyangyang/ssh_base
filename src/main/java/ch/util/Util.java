 ...
 * 常用算法或者通用类
 * 
 * @author zhaosimiao 2016-12-7 12:30:28
 *
 */
public class Util {
	...
	/**
	 * 按绝对长度截取字符串，并在后面加pointLeng个点，汉字占两位 形势：abcdef... 如果结束处刚好是半个汉字，则少取一位
	 */
	...
	public static Object beanReplaceFiter(Object instance, String[] attributes) {
		...
		for (int m = 0; m < method.length; m++) {
			...
			if (meth.getName().startsWith("get") && meth.getReturnType().getSimpleName().equals("String")) {
				boolean ifcontain = false;
				for (int i = 0; i < outFiter.length; i++) {
					if (meth.getName().toLowerCase().endsWith(outFiter[i].toLowerCase())) {
						ifcontain = true;
						break;
					}
				}
				if (!ifcontain) {
					...
					try {
						...
						if (text != null && text.length() > 0) {// 如果text是有值的
							...
							// 再set回去
							Method setMethod = instance.getClass().getMethod("set" + meth.getName().substring(3),
	 ...
	 * @return
	 */
	public static String getPublicValue(String baseName, String key) {
	...
	public static void clearCookies(HttpServletResponse response) {
		/** 写cookie操作 start **/
		String domain = ".xatrm.com";
		Cookie cUserId = new Cookie("userId", "");
		Cookie cUserCode = new Cookie("userCode", "");
		Cookie cUserPwd = new Cookie("userPwd", "");
		Cookie cUserType = new Cookie("userType", "");
		/*
		 * cuserId.setMaxAge(24*60*60); cUserCode.setMaxAge(24*60*60);
		 * cUserPwd.setMaxAge(24*60*60);
		 */
		cUserId.setPath("/");
		cUserId.setDomain(domain);

		cUserCode.setPath("/");
		cUserCode.setDomain(domain);

		cUserPwd.setPath("/");
		cUserPwd.setDomain(domain);

		// 写COOKIE到硬盘
		response.addCookie(cUserId);
		response.addCookie(cUserCode);
		response.addCookie(cUserPwd);
		response.addCookie(cUserType);
		/** 写cookie操作 end **/
	}
	 ...
	 *            str
	 * @return String str
	 * @author zdp date : 2010-9-14 上午09:56:15
	 */
