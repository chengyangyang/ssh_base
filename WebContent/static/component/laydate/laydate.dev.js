...
var config =  {
    path: '../../../../', //laydate所在路径
    skin: 'laydate-default.min.css', //初始化皮肤
...
var as = ['laydate_box', 'laydate_void', 'laydate_click', 'LayDateSkin', 'skins/', '/laydate.min.css'];
...
win.laydate = function(options){
    ...
    // 1. 如果当前页面有多个日期选择框，则将上一个图标先初始化即去掉选中的状态（必须在run之前，否则Dates.elem又要被重新赋值）
    if (Dates.elem) {
    	$(Dates.elem).next().removeClass('laydate-icon-choose');
    }
    Dates.run(options);
    
    // 2.初始化，将“今天”按钮亮起 update by shangwenshu
    var clickedDate = Dates.elem[as.elemv];
    if (clickedDate == laydate.now()) {
        $("#laydate_today").addClass('choose_btn');
    } else {
    	$("#laydate_today").removeClass('choose_btn');
    }
    // 3.将输入框的图标变成红色
    $(options.elem).next().addClass('laydate-icon-choose');
...
Dates.use = function(lib, id){
    ...
    link.type = 'text/css';
    link.rel = 'stylesheet';
    link.href = Dates.getPath + lib + id;
    id && (link.id = id);
...
Dates.query = function(node){
    ...
    if(!elemId){
        ...
        find = elemId.getElementsByClassName ? elemId.getElementsByClassName(child) : elemId[tags]('*');
    ...
    if(!Dates.box){
        ...
        div.innerHTML =  log.html = '<div class="laydate_top">'
          +'<div class="laydate_ym laydate_y" id="laydate_YY">'
            +'<a class="laydate_choose laydate_chprev laydate_tab"><cite class="left_arrow"></cite></a>'
            +'<input id="laydate_y" readonly><label></label>'
            +'<a class="laydate_choose laydate_chnext laydate_tab"><cite class="right_arrow"></cite></a>'
            +'<div class="laydate_yms">'
              +'<a class="laydate_tab laydate_chtop"><cite class="top_arrow"></cite></a>'
              +'<ul id="laydate_ys"></ul>'
              +'<a class="laydate_tab laydate_chdown"><cite class="bottom_arrow"></cite></a>'
            +'</div>'
          +'</div>'
...
Dates.close = function(){
    ...
    // 关闭日期控件时将图标颜色置为灰色 update by shangwenshu
    $(Dates.elem).next().removeClass("laydate-icon-choose");
...
Dates.creation = function(ymd, hide){
    ...
    if(typeof($(Dates.elem).valid) != "undefined"){
    	$(Dates.elem).valid();
    }
...
Dates.init = (function(){
    Dates.use('css/component/','laydate.min.css');
    Dates.use('css/component/', 'laydate-custom.min.css');
...
/*function chgSkin(lib) {
    Dates.skinLink.href = Dates.getPath + 'css/component/' + lib;
};*/
